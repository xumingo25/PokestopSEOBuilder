const upstreamBaseUrl = 'https://api.tcgdex.net/v2/en/sets';
...
export default async function handler(request: any, response: any): Promise<void> {
  ...
  try {
    ...
    const setId = String(request.query?.id ?? '').trim();

    if (!setId) {
      response.status(400).json({ error: 'Falta el id de la expansion.' });
      return;
    }

    const upstreamUrl = upstreamBaseUrl + '/' + encodeURIComponent(setId) + buildUpstreamSearch(requestUrl);
